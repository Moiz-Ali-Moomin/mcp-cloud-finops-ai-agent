    ...
    def compare(
        ...
        if result.cost_increase_pct > cost_threshold_pct:
            ...
            result.details.append(
                f"Cost increased by {result.cost_increase_pct:.2f}% (Threshold: {cost_threshold_pct}%)"
            )
        ...
        if result.risk_score_change > 0:
            result.details.append(
                f"Risk score increased by {result.risk_score_change:.2f}"
            )

        # 3. New Anomalies
        base_anomalies = {
            a.get("id") for a in baseline.get("analytics", {}).get("anomalies", [])
        }
        ...
        if fail_on_policy and curr_violations > 0:
            result.is_regression = True
            result.details.append(
                f"Policy violations detected: {curr_violations} (Fail-on-policy is ACTIVE)"
            )
        elif result.new_violations > 0:
            result.details.append(
                f"New policy violations detected: {result.new_violations}"
            )
