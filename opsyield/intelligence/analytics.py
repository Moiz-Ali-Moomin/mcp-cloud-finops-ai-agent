...
class ComparisonEngine:
    def compare_providers(self, analysis_results: List[Any]) -> Dict[str, Any]:
        ...
        for res in analysis_results:
            ...
            comparison["total_spend_by_provider"][provider] = res.summary.get(
                "total_cost", 0
            )
            comparison["resource_count_by_provider"][provider] = res.summary.get(
                "resource_count", 0
            )
            ...
            if res.summary.get("resource_count", 0) > 0:
                comparison["efficiency_by_provider"][provider] = res.summary.get(
                    "total_cost", 0
                ) / res.summary.get("resource_count", 0)
...
class ForecastEngine:
    def forecast_spend(
        self, daily_history: List[Dict[str, Any]], days_ahead: int = 30
    ) -> Dict[str, Any]:
        """
