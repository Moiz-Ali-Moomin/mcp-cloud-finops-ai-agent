...
3.  **Orchestrator**: Acts as the central nervous system. It routes incoming requests to the appropriate cloud provider implementations.
4.  **Collectors / Providers**: These are specific implementations tailored to different cloud APIs (AWS Cost Explorer, GCP BigQuery Billing Export, Azure Cost Management).
5.  **Analysis Engine**: Processes the raw data fetched by the collectors to uncover insights (e.g., cost spikes, underutilized assets).
6.  **Optimization Engine**: Generates specific, actionable recommendations based on the analysis.
...
5.  **Data Collection**: The assigned Provider Collector calls the respective provider’s API.
6.  **Processing**: The Analysis Engine transforms the raw JSON into cost trends and identifying metadata.
7.  **Response Synthesis**: The server returns a structured Markdown/JSON payload back to the AI Client.

## Orchestration Pipeline

The orchestrator sits at the heart of the system. It uses a Factory pattern to instantiate the right cloud client dynamically based on the requested `provider` or the ambient configuration. It is responsible for cross-cloud aggregation if a request queries multiple providers simultaneously.
